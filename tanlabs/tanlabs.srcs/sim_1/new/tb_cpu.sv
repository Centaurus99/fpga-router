...
module tb_cpu #(
    ...
    wire [15:0] leds;  // 16位LED，输出时 1 点亮
    ...
    wire ext_ram_ce_n;  // ExtRAM 片选使能，低有效
    ...
    // Windows 系统要注意路径分隔符的转义，例如 "D:\\foo\\bar.bin"
    parameter BASE_RAM_INIT_FILE = FULL_TEST ? "testall.bin" : "gpio_test.bin"; // BaseRAM 初始化文件，请修改为实际的绝对路径
    ...
    initial begin
        ...
        if (FULL_TEST) begin
            fd = $fopen("testall_trace.txt", "r");
        end else begin
            #100000;
            // G 0x80001000 UTEST_SIMPLE
    ...
    always_ff @(posedge tb_clk) begin
        if (reset_btn || !FULL_TEST) begin
            count   <= 0;
            last_pc <= 0;
        end else begin
            if (tb_valid && last_pc != tb_pc) begin
                last_pc <= tb_pc;
                count = count + 1;
                $display("%d: %08x %x %x %08x", count, tb_pc, tb_we, tb_waddr, tb_wdata);
                ret = $fscanf(fd, "%x%x%x%x", trace_pc, trace_we, trace_waddr, trace_wdata);
                if (ret != 4) begin
                    $display("INPUT FILE ERROR! data is not enough!");
                    $finish;
                end else begin
                    if (trace_pc != tb_pc) begin
                        $display("ERROR! pc is not match!");
                        $finish;
                    end
                    if (trace_pc != 32'h80002a90 && trace_pc != 32'h80002ad0) begin // FIXME: 暂时忽略 fence.i 指令
                        if (trace_we != (tb_we & (|tb_waddr))) begin
                            $display("ERROR! we is not match!");
                            $finish;
                        end
                        if (trace_we == 1 && trace_waddr != tb_waddr) begin
                            $display("ERROR! waddr is not match!");
                            $finish;
                        end
                        if (trace_we == 1 && trace_wdata != tb_wdata) begin
                            $display("ERROR! wdata is not match!");
                            $finish;
                        end
                    end
                    if (trace_pc == 32'h80000004) begin
                        $display("Done!");
                        $finish;
                    end
                end
